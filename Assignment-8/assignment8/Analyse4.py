...
for file in FileName:
	...
	for iter_ in range(0, len(UniqueVals)):
		...
		temp.append( np.var( Time[start : 100 + start ] ) )
		# temp.append( np.std( Time[start : 100 + start ] ) / np.mean( Time[start : 100 + start ] ) )
		# temp.append( 1/np.var( Time[start : 100 + start ] ) * Time_1[  UniqueVals[iter_] ] * Time_1[  UniqueVals[iter_] ]   )

		SpeedUpOutput.append(temp)	
		start= start + 100

	np.savetxt( 'D4'+file[1:], SpeedUpOutput )